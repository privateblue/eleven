...
object ToJs {
  ...
  implicit val indexToJs =
    new ToJs[Index] {
      def toJs(i: Index) = i.i
    }

  implicit val directionToJs =
    new ToJs[Direction] {
      def toJs(d: Direction) = d.d
    }

  implicit val colorToJs =
    new ToJs[Color] {
      def toJs(c: Color) = c.c
    }

  implicit val edgeToJs =
    new ToJs[Tuple2[Index, Index]] {
  ...
  implicit val scoreToJs =
    new ToJs[Score] {
      def toJs(s: Score) = s.s
    }

  implicit val playerToJs =
    new ToJs[Player] {
      def toJs(p: Player) = p.p
    }

  implicit val gameToJs =
    new ToJs[Game] {
